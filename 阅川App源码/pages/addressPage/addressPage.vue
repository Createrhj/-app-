<template>
	...
	<scroll-view class="content" scroll-y>	
		<view class="addressInfor" v-for="userAddress in userAddress" :key="userAddress.name">
			...
			<view class="inforRight">
				<view class="inforMain">
					...
					<uni-icons style="float: right;margin-left: 15px;margin-right: -20px;" type="closeempty" size="20" @click="delAddress(userAddress)"></uni-icons>
				</view>
				<view class="inforMain">
	...
	export default {
		...
		methods: {
			...
			delAddress(userAddress){
				MarketApi.delAddress(userAddress).then(res=>res.data).then(res=>{
					uni.showToast({
						title: `删除地址成功`,
						icon: 'none'
					})
